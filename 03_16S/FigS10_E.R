...
read_and_combine_txt_files <- function(folder_path) {
  ...
  for (file in txt_files) {
    ...
    cycles <- str_extract(file_name, "(?<=_)\\d+(?=\\.txt)") 
    

    bio <- str_extract(file_name, "\\d(?=_)") 
...
folder_path <- "/2.split_index/splited_txt_file"#路径最后面不带"/"号
...
write.csv(ratio_data,file = paste0(outpath,"S10_e_data.csv"),row.names=FALSE)


S10e_data <- ratio_data%>%
...
S10e_data$cycles <- factor(S10e_data$cycles)


S10e_f <- ggplot(S10e_data,aes(x=cycles,y=ratio_mean,fill=cycles))+
...
ggsave(S10e_f,filename = paste0(outpath,"S10e_f.pdf"),
       width = 8.5,
       height = 8.5,units = "cm")
