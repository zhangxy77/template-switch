...
cy20 <- read.table("16S_1ng_20.txt")%>%
  mutate(cycles=20)
cy25 <- read.table("16S_1ng_25.txt")%>%
  mutate(cycles=25)
cy30 <- read.table("16S_1ng_30.txt")%>%
  mutate(cycles=30)
...
S10d_f <- ggplot(major_data,aes(x=cycles,y=major_freq,fill=cycles))+
...
save(S10d_f, file = paste0(outpath,"S10d_f_plot.RData"))

ggsave(S10d_f,filename = paste0(outpath,"S10d_f.pdf"), 
       width = 8.5,
       height = 8.5,units = "cm")
