...
def run_ccs(name):
    ...
    os.system("ccs --min-length 700 --max-length 1800 -j %s --min-passes 5 %s.bam %s_ccs.bam"%(threads,name[:-4],name[:-4]))
    os.system("samtools view -h -@ 50 -o %s_ccs.sam %s_ccs.bam"%(name[:-4],name[:-4]))
